...
dm.insert_task("grp1", "NIE", "Read chapter 3", datetime(2026,8,14,23,0), 200  )
        ...
        while i != len(dupe_workblocks):
            ...
            #If the task fits workblock partially, we split it into parts: the one which occupies work block, the task with remaining time
                ...
                if ((dupe_workblocks[i][0]-dupe_workblocks[i][1]).total_seconds()/60)*(-1) != 0:
                    #dm.insert_task(tasks[j][4], tasks[j][0], tasks[j][0], tasks[j][2], <duration>) - tasks[j][1]-((dupe_workblocks[i][0]-dupe_workblocks[i][1]).total_seconds()/60)*(-1)
                    dm.insert_task(tasks[j][4], tasks[j][0], tasks[j][0], tasks[j][2], ((dupe_workblocks[i][0]-dupe_workblocks[i][1]).total_seconds()/60)*(-1))
                    tasks.insert(j+1, (tasks[j][0], ((dupe_workblocks[i][0]-dupe_workblocks[i][1]).total_seconds()/60)*(-1), tasks[j][2], tasks[j][3], tasks[j][4]))

                    dm.insert_task(tasks[j][4], tasks[j][0], tasks[j][0], tasks[j][2], tasks[j][1]-((dupe_workblocks[i][0]-dupe_workblocks[i][1]).total_seconds()/60)*(-1))
                    tasks.insert(j+2, (tasks[j][0], tasks[j][1]-((dupe_workblocks[i][0]-dupe_workblocks[i][1]).total_seconds()/60)*(-1), tasks[j][2], tasks[j][3], tasks[j][4]))
